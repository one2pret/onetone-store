...
import {
  ArrowRight, Truck, Shield, CreditCard, RotateCcw, Tag, ShoppingBag,
} from 'lucide-react';
...
export default async function StorePage({ params }: Props) {
  ...
  // For now only Onetone has full boutique content.
  // Future: generic store page for multi-seller.
  if (slug !== 'onetone') {
    return (
      <div className="bg-background min-h-screen flex items-center justify-center">
        <div className="text-center px-4">
          <p className="text-foreground font-semibold text-lg mb-1">{store.name}</p>
          <p className="text-muted-foreground text-sm">
            Halaman toko sedang disiapkan.
          </p>
          <Link href="/" className="mt-4 inline-block text-sm text-foreground underline underline-offset-2">
            Kembali ke beranda
          </Link>
        </div>
      </div>
  ...
  const [
    ...
    getFeaturedProducts(8, true),
    getActiveProducts({ limit: 4, forStore: true }),
    getBestSellerProducts(4, true),
    getActiveProducts({ limit: 4, categorySlug: 'olahraga', forStore: true }),
    getActiveBanners(),
    getHeroConfig(),
    getEditorialBreaks(),
  ]);
    ...
    <div className="bg-background">
      ...
      <section id="unggulan" className="py-10 md:py-16 scroll-mt-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-6 md:mb-10">
            <div>
              <h2 className="text-xl md:text-3xl font-bold text-foreground text-balance">Produk Unggulan</h2>
              <p className="text-sm text-muted-foreground mt-1">Pilihan terbaik minggu ini</p>
            </div>
            <Button asChild variant="ghost" size="sm" className="text-primary hover:text-primary-hover hover:bg-accent text-xs md:text-sm shrink-0">
              <Link href="/products">Lihat Semua <ArrowRight className="w-3.5 h-3.5 ml-1" /></Link>
            </Button>
          </div>
          {featuredProducts.length > 0 ? (
            ...
            <div className="py-16 flex flex-col items-center gap-4 text-center">
              ...
              <p className="text-sm font-medium text-foreground">Belum ada produk unggulan.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="pb-10 md:pb-16 bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-6 md:mb-10">
            <div>
              <h2 className="text-xl md:text-3xl font-bold text-foreground text-balance">New Arrival</h2>
              <p className="text-sm text-muted-foreground mt-1">Koleksi baru minggu ini</p>
            </div>
            <Button asChild variant="ghost" size="sm" className="text-primary hover:text-primary-hover hover:bg-accent text-xs md:text-sm shrink-0">
              <Link href="/products?sort=newest">Lihat Semua <ArrowRight className="w-3.5 h-3.5 ml-1" /></Link>
            </Button>
          </div>
          {newArrivalProducts.length > 0 ? (
            ...
            <div className="py-16 flex flex-col items-center gap-4 text-center">
              ...
              <p className="text-sm font-medium text-foreground">Belum ada koleksi terbaru.</p>
            </div>
          )}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-xl border border-border bg-card px-6 py-5 md:px-10 md:py-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground text-xs md:text-sm mt-1">Gratis. Daftar sekali, nikmati selamanya.</p>
            </div>
            <Button asChild size="sm" className="shrink-0 bg-primary hover:bg-primary-hover text-primary-foreground rounded-lg px-5 shadow-md shadow-primary/20">
              <Link href="/register">Daftar Sekarang <ArrowRight className="w-3.5 h-3.5 ml-1.5" /></Link>
            </Button>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 md:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-6 md:mb-10">
            <h2 className="text-xl md:text-2xl font-bold text-foreground text-balance">Kenapa Belanja di Onetone?</h2>
          </div>
          ...
          <div className="hidden md:grid md:grid-cols-3 gap-5">
            {whyFeatures.map((item) => (
              <div key={item.title} className="p-6 rounded-xl border border-border bg-card hover:border-primary/30 transition-colors duration-200">
                <div className="w-10 h-10 rounded-lg bg-primary/10 text-primary flex items-center justify-center mb-4">
                  <item.icon className="w-5 h-5" />
                </div>
                <h3 className="text-base font-semibold text-foreground mb-2 text-balance">{item.title}</h3>
      ...
      <section className="py-14 md:py-20 bg-surface border-t border-border">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            <Button asChild size="lg" className="bg-primary hover:bg-primary-hover text-primary-foreground rounded-lg px-8 h-11 md:h-12 font-semibold shadow-lg shadow-primary/20">
              <Link href="/products">Mulai Belanja <ArrowRight className="w-4 h-4 ml-2" /></Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="rounded-lg px-8 h-11 md:h-12 border-border text-foreground hover:bg-muted/50">

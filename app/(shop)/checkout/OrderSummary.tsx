...
export function OrderSummary({ cart: initialCart, subtotal: initialSubtotal }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-100 p-6 sticky top-24">
      <h2 className="text-lg font-semibold text-slate-800 mb-4">Pesanan Anda</h2>
            ...
            <div key={item.id} className="py-3 first:pt-0">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center border border-slate-200 rounded-md">
                  ...
                  <span className="w-7 text-center text-xs font-medium text-slate-700">
                    {item.quantity ?? 1}
                  </span>
                  <button
                    onClick={() => handleQuantityChange(item.id, (item.quantity ?? 1) + 1)}
                ...
                <span className="text-xs text-slate-400">@ {formatRupiah(unitPrice)}</span>
                <button

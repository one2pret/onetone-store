    ...
    <>
      ...
      <header className="sticky top-0 z-40 h-16 bg-card border-b border-border flex items-center justify-between px-4 md:px-6">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative" ref={menuRef}>
            ...
            {/* Dropdown */}
            {menuOpen && (
              <div className="absolute right-0 mt-1 w-56 bg-popover rounded-xl shadow-xl shadow-black/20 border border-border py-1.5 z-50 animate-fadeIn">
                ...
                <button
                  ...
                  className="flex items-center gap-2.5 w-full px-3 py-2.5 text-sm text-danger hover:bg-danger/10 transition-colors"
                >
      ...
      {logoutDialogOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          />
          <div className="relative bg-card border border-border rounded-2xl shadow-2xl shadow-black/40 w-full max-w-sm mx-4 p-6 animate-fadeInUp">
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-lg font-bold text-foreground mb-1">Keluar dari Admin?</h3>
              <p className="text-sm text-muted-foreground mb-6">Kamu akan keluar dari panel admin.</p>
              <div className="flex gap-3 w-full">
                <button
                  onClick={() => setLogoutDialogOpen(false)}
                  className="flex-1 px-4 py-2.5 text-sm font-medium text-foreground bg-secondary hover:bg-accent border border-border rounded-xl transition-colors"
                >
                  Batal
                </button>
                <form action={adminLogout} className="flex-1">
                  <button
                    type="submit"
                    className="w-full px-4 py-2.5 text-sm font-medium text-white bg-danger hover:bg-danger/90 rounded-xl transition-colors flex items-center justify-center gap-2"
                  >
                    <LogOut className="w-4 h-4" />
                    Ya, Keluar
                  </button>

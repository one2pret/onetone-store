...
// Client component — load Google API script, buka Picker, kirim fileIds ke server action
...
interface Props {
  productId: number;
  onImported?: () => void; // callback untuk refresh gambar setelah import
}
...
export function GoogleDrivePicker({ productId, onImported }: Props) {
  ...
  const openPicker = useCallback(async () => {
    ...
    try {
      ...
      // 2. Load picker module
      await new Promise<void>((resolve) => {
        window.gapi.load("picker", () => resolve());
      });

      // 3. Minta access token via GIS (Google Identity Services)
  ...
  // ── Handle hasil pilihan Picker ─────────────────────────────────────────────

  const handlePickerCallback = useCallback(
    (data: any, accessToken: string) => {
      ...
      if (action === window.google.picker.Action.PICKED) {
        ...
        startTransition(async () => {
          ...
          if (result.success) {
            ...
            toast.error(
              result.errors[0] ?? "Gagal import dari Drive"
            );
          }
        });
      }
